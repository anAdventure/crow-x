...
//        return actWorkflowFormDataService.insertActWorkflowFormDatas(acwfds);
    ...
    @Override
    public   org.activiti.api.process.model.ProcessInstance addTasks(AddTaskDTO addTaskDTO) {
        StartProcessPayloadBuilder startProcessPayloadBuilder = ProcessPayloadBuilder
                .start()
                // 流程key
                .withProcessDefinitionKey(addTaskDTO.getKey())
                // 流程标题
                .withName(addTaskDTO.getTitle())
                // 指定业务Id
                .withBusinessKey(addTaskDTO.getId());

        if(CollectionUtils.isNotEmpty(addTaskDTO.getVariables())){
            startProcessPayloadBuilder.withVariables(addTaskDTO.getVariables());
        }

        org.activiti.api.process.model.ProcessInstance processInstance = processRuntime.start(startProcessPayloadBuilder.build());
//        ProcessInstance exection = runtimeService.startProcessInstanceByKey(reProcKey,"业务key");
